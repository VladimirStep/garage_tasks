...
RSpec.describe Task, type: :model do
  ...
  describe 'scopes' do
    it '.statuses_ordered_asc get all statuses, not repeating, alphabetically ordered' do
    ...
    it '.begins_with_letter get the tasks for all projects having the name beginning with "x" letter' do
      ...
      expect(Task.begins_with_letter('M')).to match_array(Task.find(sample_task_ids))
    end

    it '.with_duplicate_names get the list of tasks with duplicate names, order alphabetically' do
      20.times { create(:task) }
      ...
      expect(Task.with_duplicate_names).to eq(['Task name 1', 'Task name 2', 'Task name 3'])
    end

    it '.exact_match_from_project get the list of tasks having several exact matches of both name and status from the project "x" order by matches count' do
      ...
      Project.find_each do |project|
        ...
        Task.where(id: ids_1).update_all(name: 'Task name 1', status: 'in-progress')
        Task.where(id: ids_2).update_all(name: 'Task name 2', status: 'completed')
      end

      expect(Task.exact_match_from_project('Garage')).to eq([[['Task name 2', 'completed'], 2], [['Task name 1', 'in-progress'], 4]])
    end
  end
end
